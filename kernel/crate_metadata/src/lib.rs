    ...
    pub fn deep_copy(
        ...
        for new_sec in new_sections.values() {
            ...
            let new_sec_mapped_pages = match new_sec.typ {
                SectionType::Text    => new_text_pages_locked.as_mut().ok_or("BUG: missing text pages in newly-copied crate")?,
                SectionType::Rodata |
                SectionType::GccExceptTable | 
                SectionType::EhFrame => new_rodata_pages_locked.as_mut().ok_or("BUG: missing rodata pages in newly-copied crate")?,
                SectionType::Data |
                SectionType::Bss     => new_data_pages_locked.as_mut().ok_or("BUG: missing data pages in newly-copied crate")?,
            };
